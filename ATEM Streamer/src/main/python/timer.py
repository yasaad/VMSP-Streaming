...
class TimeSpinner(QWidget):
    def __init__(
        self, title, min, max, connected_function=None, step=1, value=0, *args, **kwargs
    ):
...
class Timer(QWidget):
    def __init__(self, stop_stream_callback, showTimerButton, *args, **kwargs):
        ...
        self.timer_button.clicked.connect(
            lambda: self.stop() if self.running else self.start()
        )
        ...
        self.hourSelector = TimeSpinner(
            "Hours", 0, 23, connected_function=self.__timer_hours_minutes_changed
        )
        self.minuteSelector = TimeSpinner(
            "Minutes", 0, 59, connected_function=self.__timer_hours_minutes_changed
        )
    ...
    def setTime(self, time):
        ...
        self.hourSelector.setValue(time // 3600)
        remaining = time % 3600
        self.minuteSelector.setValue(remaining // 60)
        self.__update_timer()
    ...
    def __timer_hours_minutes_changed(self):
        self.remaining_time = (
            self.hourSelector.getValue() * 3600 + self.minuteSelector.getValue() * 60
        )
        self.__update_timer()
