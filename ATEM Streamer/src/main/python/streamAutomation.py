...
class StreamAutomation:
        ...
        if not credentials or not credentials.valid:
            if credentials and credentials.expired and credentials.refresh_token:
                ...
                credentials = flow.run_local_server(
                    port=8080, authorization_prompt_message="Authorizing Account"
                )
    ...
    def setThumbnail(self, video_id, imagePath):
        ...
        request = self.youtube.thumbnails().set(
            videoId=video_id, media_body=MediaFileUpload(imagePath)
        )
    ...
    def checkBroadcastStatus(self, broadcast_id):
        # pylint: disable=maybe-no-member
        request = self.youtube.liveBroadcasts().list(
            part="snippet,contentDetails,status", prettyPrint=True, id=broadcast_id
        )
        response = request.execute()
    ...
    def bindBroadcast(self, broadcast_id):
        ...
        request = self.youtube.liveBroadcasts().bind(
            id=broadcast_id, part="snippet", streamId=self.STREAM_ID
        )
        response = request.execute()
        return response
    ...
    def checkLiveStreamStatus(self, stream_id=True):
        ...
        request = (
            self.youtube.liveStreams().list(
                part="snippet,cdn,contentDetails,status", id=self.STREAM_ID
            )
            if stream_id
            else self.youtube.liveStreams.list(
    ...
    def endBroadcast(self, broadcast_id):
        ...
        if status != "live":
            ...
            request = self.youtube.liveBroadcasts().transition(
                part="snippet,status", broadcastStatus="complete", id=broadcast_id
            )
            response = request.execute()
            return response
    ...
    def turnOnPowerSwitch(self):
        ...
        switch = dlipower.PowerSwitch(
            hostname="192.168.1.33", userid="admin", password="1234"
        )
        switch.on("Mixer")
        # switch.on("ATEM") # cycling thise seems to put the camera's to the menu screen so keeping on for now
        switch.on("Amp")
    ...
    def turnOffPowerSwitch(self):
        ...
        switch.off("Mixer")
        # switch.off("ATEM") # cycling thise seems to put the camera's to the menu screen so keeping on for now
        switch.off("Amp")

    def setStreamKey(self, scheduled=False):
        if self.switcher.waitForConnection(infinite=False):
            if scheduled:
                self.switcher.setMacroAction(
                    self.switcher.atem.macros.macro11,
                    self.switcher.atem.macroActions.runMacro,
                )
                print("Set to scheduled stream key")
            else:
                self.switcher.setMacroAction(
                    self.switcher.atem.macros.macro10,
                    self.switcher.atem.macroActions.runMacro,
                )
                print("Set to autostreamer stream key")
        else:
    ...
    def startATEMStream(self):
        if self.switcher.waitForConnection(infinite=False):
            self.switcher.setMacroAction(
                self.switcher.atem.macros.macro8,
                self.switcher.atem.macroActions.runMacro,
            )
    ...
    def stopATEMStream(self):
        if self.switcher.waitForConnection(infinite=False):
            self.switcher.setMacroAction(
                self.switcher.atem.macros.macro5,
                self.switcher.atem.macroActions.runMacro,
            )
    ...
    def startOBS(self):
        if "obs64.exe" in (p.name() for p in psutil.process_iter()):
            ...
            time.sleep(1)
        OBS_DIR = os.path.abspath(
            os.path.expanduser(r"C:\\Program Files\\obs-studio\\bin\\64bit\\")
        )
